...
"""
    ...
    if(query_param.expected_length == 0):
        ...
        sys_prompt_temp = PROMPTS["global_reduce_rag_response_withlength"]
        response = await use_model_func(
            query,
            sys_prompt_temp.format(
                report_data=points_context, response_type=query_param.response_type , expected_length = query_param.expected_length
            ),
        )
    return response
